...
for i in {1..$COV}
do
    awk 'BEGIN{id=1;ii=0}{prev="";if($12 != "N"){if(prev!=$1){prev=$1;ii=1;} gap_id=0+$1;ii=ii+1 ; if(ii%cov==the_i){printf(">seq_id_%d_%d\n%s\n",gap_id,id,$12);}id+=1;}}' cov=$COV the_i=$i \
...
cat "$OUT_PREFIX\.*\.fasta" $OUT_PREFIX.ont.fasta
